...
export async function wareHouseLocation(): Promise<Location[]> {
  try {
    ...
    const res = await client.request<{ results: Location[] }>({
      url: `${process.env.NEXT_PUBLIC_MAP_SUCHNAVALI_VISUALIZER_BASE_URL}/serviceableWarehouse/location`,
      method: "GET",
    });
...
export async function getwareHouseLocations(): Promise<wareHouse[]> {
  try {
    ...
    const res = await client.request<{ results: wareHouse[] }>({
      url: `${process.env.NEXT_PUBLIC_MAP_SUCHNAVALI_VISUALIZER_BASE_URL}/serviceablePincode/location`,
      method: "GET",
    });
    ...
    while (page <= totalPages) {
      try {
        const res = await client.request<{ results: any, Info: { totalPages: number } }>({
          url: `${process.env.NEXT_PUBLIC_MAP_SUCHNAVALI_VISUALIZER_BASE_URL}/orderData/location?from_date=${from_date}&to_date=${to_date}&metric=${metric}&page=${page}&limit=${limit}`,
          method: "GET",
        });
