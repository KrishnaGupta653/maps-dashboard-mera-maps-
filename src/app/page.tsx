...
export default function Dashboard() {
  ...
  const handleToggle = useCallback(async (
    ...
    if (isSelected && shouldAutoCollapse()) {
    ...
    <div className="relative w-full h-screen bg-black text-black overflow-hidden">
      ...
      <div className={`absolute top-2 left-2 z-50 ${isFullscreen ? 'fixed' : ''}`}>
        <button
          ...
          className="flex items-center justify-center w-10 h-10 bg-white/15 backdrop-blur-md rounded-lg border border-white/10 hover:bg-white/20 transition-colors"
        >
          {isMenuOpen ? (
      ...
      <div className={`${isFullscreen ? 'fixed' : 'absolute'} top-14 left-2 z-50 transition-all duration-300  ${
        isMenuOpen ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-4 pointer-events-none'
      }`}>
        <div className="bg-white/15 backdrop-blur-md rounded-lg border border-white/10 p-4 min-w-[280px] max-h-[calc(100vh-80px)] overflow-y-auto no-scrollbar">
